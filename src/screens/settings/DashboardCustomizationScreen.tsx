...
import { useTheme, useDashboard } from '@/contexts';
import { Card, Button } from '@/components/ui';
import { DashboardCard } from '@/types';

const DashboardCustomizationScreen: React.FC = () => {
  const { colors } = useTheme();
  const { config, toggleCardVisibility, reorderCards, updateCardColor, resetToDefault } = useDashboard();
  const insets = useSafeAreaInsets();
  const navigation = useNavigation();

  const [cards, setCards] = useState<DashboardCard[]>(config.cards);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Dashboard Settings</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.infoBox, { backgroundColor: colors.info + '10' }]}>
          ...
          <Text style={[styles.infoText, { color: colors.textMuted }]}>
            Long-press on any card in your dashboard to quickly toggle visibility or change its color.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
